...
def main():
    ...
    parser.add_argument('--folder', default='data/maxdeg/tmp', type=str, help='run file')
    ...
    parser.add_argument('--run', default='maxdeg', type=str, help='run file')
    parser.add_argument('--prefix', default='max_degree', choices=['tree', 'random_graph', 'max_degree', 'shortest_path_pred', 'shortest_path_length'], type=str, help='task prefix')
    args = parser.parse_args()
    Path(args.folder).mkdir(parents=True, exist_ok=True)
    output = './%s/%s_dim%s_sampling:%s_graph:%s_N%s_%s_C%s_test_G%s_N%s_%s_C%s.pickle' %(args.folder, args.prefix, args.node_dim, args.sampling, args.train_graph, args.train_min_n, args.train_max_n, args.train_color, args.test_graph, args.test_min_n, args.test_max_n, args.test_color)
    args.run = 'maxdeg'
