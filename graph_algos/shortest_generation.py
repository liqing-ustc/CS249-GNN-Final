...
def main():
    ...
    parser.add_argument('--folder', default='data/shortest/tmp', type=str, help='run file')
    parser.add_argument('--sampling', default='uniform', type=str, help='uniform or int')
    parser.add_argument('--rp', default=0.1, type=float, help='random graph (expander) probability')
    ...
    parser.add_argument('--run', default='shortest', type=str, help='run file')
    parser.add_argument('--prefix', default='shortest', type=str, help='task prefix')
    ...
    output = './%s/rp%s_%s_dim%s_sampling:%s_graph:%sedge%s_N%s_%s_C%s_test_Edge%s_G%s_N%s_%s_C%s.pickle' % (
    args.folder, args.rp, args.prefix, args.node_dim, args.sampling, args.train_graph, args.max_weight,
    args.train_min_n, args.train_max_n, args.train_color, args.max_weight_test, args.test_graph, args.test_min_n,
    args.test_max_n, args.test_color)

    print(args)
    train = generate_shortest_paths(args, args.num_graphs, args.train_min_n, args.train_max_n, args.train_color,
                                    args.train_graph, args.max_weight)
    train = load_data(train)
    val = generate_shortest_paths(args, max(int(args.num_graphs * VAL_RATIO), 1), args.train_min_n, args.train_max_n,
                                  args.train_color, args.train_graph, args.max_weight)
    val = load_data(val)
    test = generate_shortest_paths(args, max(int(args.num_graphs * TEST_RATIO), 1), args.test_min_n, args.test_max_n,
                                   args.test_color, args.test_graph, args.max_weight_test)
    test = load_data(test)
