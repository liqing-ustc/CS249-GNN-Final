    ...
    def test_(self, input_nodes, label, print_info=False):
        with torch.no_grad():
            ...
            if self.return_correct:
                ...
                return accuracy, loss, mape_loss
...
class RN(BasicModel):
    def __init__(self, args):
        ...
        for layer in range(self.n_iter):
            if layer == 0:
                self.MLP0.append(
                    MLP(self.mlp_layer, (self.coord_size + self.add_features) * 2, self.hidden_dim, self.hidden_dim))

                if not self.all_same:
                    self.MLPdd.append(MLP(self.mlp_layer, (self.coord_size + self.add_features) * 2, self.hidden_dim,
                                          self.hidden_dim))
                    for i in range(self.n_dummy):
                        self.MLPdios[i].append(
                            MLP(self.mlp_layer, (self.coord_size + self.add_features) * 2, self.hidden_dim,
                                self.hidden_dim))
                        self.MLPodis[i].append(
                            MLP(self.mlp_layer, (self.coord_size + self.add_features) * 2, self.hidden_dim,
                                self.hidden_dim))
            else:
                ...
                if not self.all_same:
                    ...
                    for i in range(self.n_dummy):
                        self.MLPdios[i].append(
                            MLP(self.mlp_layer, self.hidden_dim * 2, self.hidden_dim, self.hidden_dim))
                        self.MLPodis[i].append(
                            MLP(self.mlp_layer, self.hidden_dim * 2, self.hidden_dim, self.hidden_dim))
    ...
    ''' One iteration/layer of RNs, call n_layer times in forward
    '''

    def reason_step(self, x_flat, layer, mb, step_layer=DEFAULT_MODE, step_type=DEFAULT_MODE, node_pair=DEFAULT_PAIR,
                    node_ind=DEFAULT_IND):
    ...
    def reason_step_fine_grain(self, x_flat, layer, mb, step_layer=DEFAULT_MODE, step_type=DEFAULT_MODE,
                               node_pair=DEFAULT_PAIR, node_ind=DEFAULT_IND):
...
def findsubsets(N, device):
    ...
    susbets = []
    for s in holder:
        susbets.append(torch.tensor(list(s)).to(device))
    return susbets
...
class NES(BasicModel):
    def __init__(self, args):
        ...
        self.add_features = args.add_features
        self.device = args.device
        self.subsets = findsubsets(self.n_objects, self.device)
        self.num_subsets = len(self.subsets)

        # LSTM for subset sum
        self.subset_LSTM = LSTM(input_size=self.coord_size + self.add_features, hidden_size=self.hidden_dim,
    ...
    def reset_g(self, mb):
        h = (
            torch.zeros(self.lstm_layer, mb, self.hidden_dim, requires_grad=True).to(self.device),
            torch.zeros(self.lstm_layer, mb, self.hidden_dim, requires_grad=True).to(self.device)
        )
        return h
    ...
    def forward(self, x_input):
        ...
        x_lstm_holder = torch.zeros(mb, self.num_subsets, self.hidden_dim).to(self.device)
...
class RRN(BasicModel):
    def __init__(self, args):
        super(RRN, self).__init__(args, 'RRN')
    ...
    def reset_g(self, mb):
        h = (
            torch.zeros(self.lstm_layer, mb, self.node_dim, requires_grad=True).to(device),
            torch.zeros(self.lstm_layer, mb, self.node_dim, requires_grad=True).to(device)
        )
        return h
...
class HRN(BasicModel):
    ...
    def reason_step(self, x_flat, layer, mb):
        ...
        x_k = x_k.repeat(1, 1, self.n_objects ** 2, 1).view(mb, self.n_objects ** 3,
                                                            -1)  # (mb, n_objects**3, feature) e.g. a*n_objects^2, b*n_objects^2...
...
class CNN_MLP(BasicModel):
    def __init__(self, args):
        ...
        self.hidden_dim = args.hidden_dim
        self.fc_output_layer = args.fc_output_layer
        self.fcout = FCOutputModel(self.fc_output_layer, self.n_objects * self.input_size, self.hidden_dim,
                                   self.answer_size)
...
class Skip_MLP(BasicModel):
    def __init__(self, args):
        ...
        self.hidden_dim = args.hidden_dim
        self.fc_output_layer = args.fc_output_layer
        self.fcout = FCOutputModel_SkipConnection(self.fc_output_layer, self.n_objects * self.input_size,
                                                  self.hidden_dim, self.answer_size)
