...
class GGNN(BasicModel):
    def __init__(self, args):
        super(GGNN, self).__init__(args, 'GGNN')
    ...
    def preprocess_neighbors_list(self, batch_graph):
        ...
        return torch.LongTensor(padded_neighbor_list).to(self.device), torch.LongTensor(padded_self_list).to(
            self.device), torch.LongTensor(graph_level_list).to(self.device)
