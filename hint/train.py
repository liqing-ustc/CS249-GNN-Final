...
from baseline_utils import HINT, HINT_collate
...
def parse_args():
    ...
    parser.add_argument('--seq2seq', type=str, default='GNN', help='the type of seq2seq: RNN for GRU, TRAN for Transformer, GNN for graph neural network')
    ...
    parser.add_argument('--n_iter', type=int, default=6, help='number of RN/RRN iterations/layers (default: 1)')
...
def evaluate(model, dataloader):
    ...
    with torch.no_grad():
        for sample in tqdm(dataloader):
            ...
            output = model(img, src, trg, src_len, tgt_len, dep)
            if model.config.seq2seq == 'GNN':
                res_pred = output.view(-1).detach().cpu().numpy()
                res_pred = np.round(res_pred).astype(int)
            else:
...
def train(model, args, st_epoch=0):
    ...
    for epoch in range(st_epoch, args.epochs):
        ...
        for sample in tqdm(train_dataloader):
            ...
            src = src.to(DEVICE)
            trg = trg.to(DEVICE)
            output = model(img, src, trg[:, :-1], src_len, tgt_len, head)
            if args.seq2seq == 'GNN':
                loss = criterion(output.view(res.shape), res.to(output.dtype).to(DEVICE))
            else:
        ...
        if ((epoch+1) % args.epochs_eval == 0) or (epoch+1 == args.epochs):
            ...
            model_path = args.output_dir + "model_%05d.p"%(epoch + 1)
...
if __name__ == "__main__":
    ...
    # train_set = HINT('train', numSamples=1000)
